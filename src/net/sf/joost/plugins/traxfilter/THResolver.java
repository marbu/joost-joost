...
public class THResolver implements TransformerHandlerResolver {
    ...
    /**
     * force usage of internal TH implementation for Xalan TH
     * (so far Xalan TH are not reusable, 
     * see bug http://issues.apache.org/bugzilla/show_bug.cgi?id=1205)
     * when this property is true (by default) then it will instantiate 
     * internal reusable version of TH, when false it will use default
     * one created by TraX factory.
     */
    ...
    public boolean available(String method) {
        ...
        return (STX_METHOD.equals(method) || XSLT_METHOD.equals(method) ||
        ...
        if ( FACTORY.getValueStr().length() > 0 ) {
            ...
            try {
                saxtf = (SAXTransformerFactory)
                        (Class.forName( FACTORY.getValueStr() )).newInstance();
                if ( log.isDebugEnabled() )
